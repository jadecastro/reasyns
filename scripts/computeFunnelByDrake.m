function [funnel,rhoMin,rho_d,info,redindx,p,c,Vxframe] = computeFunnelByDrake(tk,uk,xk,Qf,skipFun)
...
[tRed,redindx] = downsampleUniformly(tk,skipFun);
xRed = xk(redindx,:);
uRed = uk(redindx,:);
...
% also plot the ellipsoids (for testing)
% figure(20)
% hold on
% Hout = eye(2);
% n = 3;
% for j = 1:1:size(funnel.x,1)
%     tmp = inv(funnel.P(:,:,j));
%     tmp = (tmp+tmp')/2;
%     E1 = ellipsoid(funnel.x(j,:)',tmp*funnel.rho(j));
