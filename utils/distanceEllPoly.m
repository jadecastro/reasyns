function [d, status, ellPoint, polyPoint] = distanceEllPoly(E, X, flag)
%
...
%
  ...
  global ellOptions verbSav;

  verbSav = ellOptions.verbose;
...
%   end
  ...
  if isa(X, 'polytope')
    [d, status, ellPoint, polyPoint] = l_polydist(E, X);
    if nargout < 2
      clear status;
    end
    return;
...
function [d, status, ellPoint, polyPoint] = l_polydist(E, X)
%
