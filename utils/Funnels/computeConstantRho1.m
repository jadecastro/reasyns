function [rhoMin,rho_d,Info] = computeConstantRho1(ts,Ac,K,xMu,uk,regBnd,X,Eprior,P1,reg1,Xbnd1,Xin1,P2,reg2,Xbnd2,Xin2,ellBnd1,ellBnd2,ellIn1,ellIn2,H,n,isCyclic)
...
trialMat = [zeros(length(H),3);[zeros(n-length(H),1) 2*pi*ones(n-length(H),1) -2*pi*ones(n-length(H),1)]];
