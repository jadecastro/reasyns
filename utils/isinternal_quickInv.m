function res = isinternal_quickInv(E, X, s)
%
...
%
  ...
  % === Assume that the dimensions have already been worked out === 
%   dims = dimension(E);
%   m    = min(min(dims));
%   n    = max(max(dims));
%   if m ~= n
...
if (s == 'u')
    for i = 1:m
        for j = 1:n
            ...
            % === Assume rank of Q is not deficient ===
            % === Assume not ill-conditioned ===
            r = q' * Qinv * q;
