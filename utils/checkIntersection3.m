...
%         0
    ...
    if ~any(isectTmp1) && double(all(q(ii,1:length(H)) > min(vBound)) && all(q(ii,1:length(H)) < max(vBound)))  % point is inside R1.  Now check if inside corresponding reach tube
        regID(ii) = 0;
...
%             1
    ...
    elseif ~any(isectTmp2) && double(all(q(ii,1:length(H)) > min(vBound)) && all(q(ii,1:length(H)) < max(vBound)))  % point is inside R2.  Now check if inside corresponding reach tube
        regID(ii) = 1;
