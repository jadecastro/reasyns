function [varargout] = Ellipse_plot1(A, C, varargin)
%
...
elseif strcmp(Type,'3D'),
    ...
    b = 1/sqrt(D(2,2));
    c = 1/sqrt(D(3,3));
    rmpath('C:\Users\jad455\Dropbox\Research\LowLevelControllerSynthesis\ParametricVerification\ellipsoids')
    ...
    addpath('C:\Users\jad455\Dropbox\Research\LowLevelControllerSynthesis\ParametricVerification\ellipsoids')
...
if nargout == 0
    ...
    %----------------------------------------
    if strcmp(Type,'2D'),
        fill(X(1,:),X(2,:),color,'LineStyle','none','FaceAlpha',1);%0.5);
        hold on;
        %     plot(C(1),C(2),'r*');
        %    axis equal, grid
        ...
        H = surf(XX,YY,ZZ);
        set(H,'LineStyle','-','FaceColor',color)
        %    axis equal
        %    hidden off
    end
end