function V = verificationWithRhoBoundaryCondition(sys,ts,G,rho_if,varargin) 
...
%
% Adapted from the Drake toolbox.
%
...
for i=1:N
  ...
  try [T,D] = balanceQuadForm(S1,S2); catch; keyboard; end
  Ts{i}=T;
...
dts = diff(ts);
if options.max
    rho = flipud(rho_if*exp(-options.rho0_tau*(ts-ts(1))/(ts(end)-ts(1))))+max(Vmin); 
else
    rho = rho_if*exp(-options.rho0_tau*(ts-ts(1))/(ts(end)-ts(1)))+max(Vmin); 
end
rhodot = diff(rho)./dts;
...
for iter=1:options.max_iterations
  ...
  % NOTE: assume here that options.lyap_parameterization == 'rho' 
  % (i.e. i've removed functionality for the 'rhoS' option.)
  
  if options.max
      [rho,rhointegral]=maximizeRho(x,V,Vdot,L,dts,Vmin,rho_if,options);
  else
      [rho,rhointegral]=minimizeRho(x,V,Vdot,L,dts,Vmin,rho_if,options);
  end
...
% fix lagrange multipliers, minimize rho
function [rho,rhointegral]=minimizeRho(x,V,Vdot,L,dts,Vmin,rhoi,options)
  N = length(V)-1;
  prog = mssprog;
  [prog,rho] = new(prog,N,'pos');
  if size(Vmin,2) > size(Vmin,1)
      Vmin = Vmin';
  end
  rho = [rhoi;rho]+Vmin;
...
%  prog = sedumi(prog,ones(1,N)*slack-rhointegral,1,struct());
  [prog,info] = sedumi(prog,rhointegral,1); %1,struct());
  if (info.numerr>1)
    keyboard;
...
function [rho,rhointegral]=maximizeRho(x,V,Vdot,L,dts,Vmin,rhof,options)
  N = length(V)-1;
  prog = mssprog;
  [prog,rho] = new(prog,N,'pos');
  if size(Vmin,2) > size(Vmin,1)
...
%      prog.sos = L2;
  ...
  for i=fliplr(1:N)
    if (slack{i}>1e-4 || info{i}.pinf~=0 || info{i}.dinf~=0) & ~options.max
      if (length(x)~=2)
        ...
        figure(2); clf; [minVmRho,~]=plotPoly(x,V{i}-rho(i));
      if (options.stability)
        figure(1); clf; [~,maxVdot]=plotPoly(x,Vdot{i});
        fprintf(1,'segment %d of %d, slack=%f,\n sampledmax(d/dt(V/rho))=%f, sampledmin(V-rho)=%f\n info:\n',i,N,maxVdot,minVmRho,slack{i});
      else
        figure(1); clf; [~,maxVdotmRhodot]=plotPoly(x,Vdot{i}-rhodot(i));
