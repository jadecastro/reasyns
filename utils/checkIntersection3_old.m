function [isect] = checkIntersection3(vBound,vObs1,vObs2,ellBound1,ellBound2,ellC1,ellC2,q)
    ...
    if ~any(isectTmp1)  % point is inside R1.  Now check if inside corresponding reach tube
        ...
        if ~isempty(ellC1)
            for mm = 1:size(ellC1,1)
                isectC(mm) = ~isinternal(ellC1{mm},q(ii,:)','u');
            end
        end
        ...
        if any(isect2) && all(isectC), Isect(ii) = 1; end  % if any of the points lie outside the intersection of multiple outgoing reach tubes, flag it
    elseif ~any(isectTmp2)  % point is inside R2.  Now check if inside corresponding reach tube
        isect1 = false;
        isectC = false;
        if ~isempty(ellC2)
            for mm = 1:size(ellC2,1)
                isectC(mm) = ~isinternal(ellC2{mm},q(ii,:)','u');
            end
        end
